...
<div ui-view="content" ng-class="sidebarCollapsed" class="admin-content" style="">
    ...
    <div class="tch-inline-form">
        <div id="test-id-add-product-btn" ng-click="form.toggle()" class="btn btn-block btn-attached fastclickable">Créez un nouveau coupon<i class="fa fa-angle-down icon-arrow-down"></i></div>
    ...
    <div ng-show="products.length > 0 || deletedPlansCount > 0" class="tch-section-content">
            ...
            <table ng-if="products.length > 0" class="tch-table draggable-products">
                ...
                <tbody ui-sortable="sortableOptions" ng-model="products" class="ng-pristine ng-untouched ng-valid ui-sortable ng-not-empty">
                    ...
                    <tr what="product" which="Free Course" ng-repeat="product in products" class="border-bottom ui-sortable-handle">
                        <td what="id"><span ng-bind="'#' + product.id">#{{$coupon->id}}</span><span class="space"></span><span class="space"></span>
                            <!---->
                        </td>
                        <td what="type"><!--img ng-src="/images/divers/icon-pricing-subscription.svg" class="tch-table-thumb" src="/images/divers/icon-pricing-subscription.svg"-->
                            ...
                            <!---->
                            <!---->
                            <!----><span ng-bind="'PRODUCT.free.name' | translate" ng-if="getPlanType(product) == 'free'">{{$coupon->code}}</span>
                            <!---->
                        </td>
                        <td what="name">{{$coupon->value}}</td>
        ...
        <div ng-class="{ 'text-center': products.length == 0, 'pull-right': products.length > 0 }" ng-if=" deletedPlansCount > 0" class="pull-right">
            <!----><a ui-sref="admin.courses.course.pricing(::{id: course.id, is_published: false})" ng-if="showPublishedPlans" ui-sref-opts="{reload:true}" class="gray-underline"
              href="/admin/courses/627895/pricing?is_published=false&amp;page=1">Afficher les coupons supprimés (1)</a>
            <!---->
            <!---->
        </div>
        <!---->
