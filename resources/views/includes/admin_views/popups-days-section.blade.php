...
<div id="popup-days-preview{{$section->id}}" modal-render="true" tabindex="-1" role="dialog" class="modal fade fastclickable in" modal-animation-class="fade" modal-in-class="in" ng-style="{'z-index': 1050 + index*10, display: 'block'}" ng-click="close($event)" modal-window="modal-window"
...
<div class="modal-dialog modal-md" ng-class="size ? 'modal-' + size : ''" style="">
    <div class="modal-content" modal-transclude="">
        <div class="modal-header text-center">
            <h4><span ng-bind="modalTitle">Publier la section "{{$section->title}}" à un nombre de jours précis, après inscription au cours</span>
                <!--
...
@if($section->drip)
<div id="edit-days-preview{{$section->id}}" modal-render="true" tabindex="-1" role="dialog" class="modal fade fastclickable in" modal-animation-class="fade" modal-in-class="in" ng-style="{'z-index': 1050 + index*10, display: 'block'}" ng-click="close($event)" modal-window="modal-window"
size="md" index="0" animate="animate" modal-animation="true" style="z-index: 1050; display: none;">
<div class="modal-dialog modal-md" ng-class="size ? 'modal-' + size : ''" style="">
    <div class="modal-content" modal-transclude="">
        <div class="modal-header text-center">
        ...
        <div class="modal-body">
            <form action="{{url('drips', $section->drip)}}" method="post" style="margin-bottom: inherit;">
                ...
                <bootstrap-decorator form="schemaForm.form[0]">
                    <div ng-class="{'has-error': form.disableErrorState !== true &amp;&amp; hasError(), 'has-success': form.disableSuccessState !== true &amp;&amp; hasSuccess(), 'has-feedback': form.feedback !== false}"
                      class="form-group schema-form-select  has-feedback"><label ng-show="showTitle()" class="control-label">Date et heure</label>
                      <input type="number" name="days" value="{{$section->drip->days}}">
                      </div>
                </bootstrap-decorator>
            <!----><button type="submit" class="tch-btn-header-primary fastclickable">Valider</button>
        </form>
...
$(".show-days-section").on('click', function(){
let id = $(this).attr('id');
$('#popup-background').css('display', 'block');
$('#popup-days-preview'+id).css('display', 'block');
...
let id = $(this).attr('id');
$('#popup-background').css('display', 'none');
$('#popup-days-preview'+id).css('display', 'none');

});

$(".edit-days-section").on('click', function(){
let id = $(this).attr('id');
$('#popup-background').css('display', 'block');
$('#edit-days-preview'+id).css('display', 'block');
...
let id = $(this).attr('id');
$('#popup-background').css('display', 'none');
$('#edit-days-preview'+id).css('display', 'none');
